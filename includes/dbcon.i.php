 ...
 * @param	$Offset		If not NULL it will be appended like $query." OFFSET ".$Order
 * @return	$sqlresult as SQL result set or NULL for no results or FALSE in case the query failed
 */
function sqlSelect($Table, $Columns = "*", $Condition = null, $Order = null, $Limit = null, $Offset = null) {
	global $sqlresult;
	
	//Create query using getSqlQuerySelect and query the query... So descriptive
	sqlQuery(getSqlQuerySelect($Table, $Columns, $Condition, $Order, $Limit, $Offset).";");
 ...
 */
function getSqlQuerySelect($Table, $Columns = "*", $Condition = null, $Order = null, $Limit = null, $Offset = null) {
	global $sqltp;
	...
	$query = "SELECT ".$Columns." FROM ".$sqltp.$Table;
...
function getSqlQueryInsert($Table, $Data, $Ignore=false) {
	...
	$sKeys = "("; $sValues = "(";
	$c = count($Data); //used to detect if we should suffix the lists with a comma for the next key / value
	foreach ($Data as $k => $v) {
		$sKeys .= $k.($c>1?', ':''); 			//append key to key list
		//$sValues .= (isInteger($v)?$v:"'".$v."'").($c>1?', ':'');
		$sValues .= "'".$v."'".($c>1?', ':''); 	//appen value quoted, we expect them to be escaped tho
		$c--;
	}
...
function getSqlQueryUpdate($Table, $Data, $Condition=null, $Order=null, $Limit=null) {
	global $sqltp;
	$query = "UPDATE ".($Table !== null ? $sqltp.$Table." SET " : "");
...
function sqlReplace($Table, $Key, $Data) {
	...
	$pquery = "SELECT * FROM `".$sqltp.$Table."` WHERE ".$Key."='".$Data[$Key]."' LIMIT 1";
